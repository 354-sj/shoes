...
# 加载训练好的模型
model_filename = r'C:\Users\Administrator\图像分类和推荐系统\resNet34_6_2_2.pkl'
with open(model_filename, 'rb') as file:
    loaded_model = pickle.load(file)
# read class_indict
json_path = r'C:\Users\Administrator\图像分类和推荐系统\class_indices.json'
...
def classify_image(img_path):
    img_tensor = load_and_preprocess_image(img_path)
    with torch.no_grad():
        output = torch.squeeze(loaded_model(img_tensor.to(device))).cpu()
...
# 数据库图像路径
db_dir = r'C:\Users\Administrator\图像分类和推荐系统\shoe'  # 数据库图像文件夹
...
if uploaded_file is not None:
    ...
    # 分类并显示结果
    class_num, probabilities = classify_image("temp.jpg")
    st.write(f"Class_name: {class_num}")
